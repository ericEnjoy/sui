        ...
        if let Some(checkpoint) = checkpoint {
            if checkpoint.epoch() == cur_epoch {
                ...
                }) = &checkpoint.summary.end_of_epoch_data
                {
                    ...
                    let executable_txns: Vec<_> = self
                        .authority_store
                        ...
                        .map(|tx| {
                            VerifiedExecutableTransaction::new_from_checkpoint(
                                tx.expect(
                                    "state-sync should have ensured that the transaction exists",
                                ),
                                epoch_store.epoch(),
                                checkpoint_sequence,
                            )
                    ...
                    let change_epoch_effects = execute_transactions_impl(
...
async fn execute_transactions(
    ...
    let checkpoint_sequence = checkpoint.sequence_number();
    let mut execution_digests = execution_digests.clone();
    let mut all_tx_digests: Vec<TransactionDigest> =
        execution_digests.iter().map(|tx| tx.transaction).collect();

    let mut executable_txns: Vec<_> = authority_store
        .multi_get_transactions(&all_tx_digests)?
        .into_iter()
        .map(|tx| {
            VerifiedExecutableTransaction::new_from_checkpoint(
    ...
    let effects = execute_transactions_impl(
        execution_digests,
...
async fn execute_transactions_impl(
    execution_digests: Vec<ExecutionDigests>,
