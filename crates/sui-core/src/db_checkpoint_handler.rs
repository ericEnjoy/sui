...
use sui_storage::object_store::util::{copy_recursively, delete_recursively};
...
impl DBCheckpointHandler {
    ...
    async fn upload_db_checkpoint_to_object_store(&self) -> Result<()> {
        ...
        let next_epoch = if let Some((last_epoch, path)) =
            remote_checkpoints_by_epoch.last_key_value()
        {
        ...
        if let Some((epoch, db_path)) = next_db_checkpoint_to_copy {
            ...
            self.output_object_store
                .put(&success_marker, bytes.clone())
                .await?;
            ...
            for (gc_epoch, gc_path) in &local_checkpoints_by_epoch {
                if *gc_epoch <= *epoch {
                    ...
                    self.input_object_store
                        .put(&upload_completed_marker, bytes.clone())
                        .await?;
                }
            }
        }
    ...
    async fn garbage_collect_old_db_checkpoints(&self) -> Result<()> {
        ...
        for (epoch, path) in local_checkpoints_by_epoch.iter() {
            ...
            match all_markers_present {
                ...
                Ok(_) => {
                    info!("Garbage collecting db checkpoint dir: {path} for epoch: {epoch}");
                    delete_recursively(
                        path,
                        self.input_object_store.clone(),
                        NonZeroUsize::new(20).unwrap(),
                ...
                Err(_) => {
                    debug!("Not ready for garbage collection yet: {path}");
                }
            }
        }
        Ok(())
